...
def enhancedPacmanFeatures(state, action):
    ...
    # Feature 1: The number of food pellets left
    foodCount = successor.getFood().count()
    features['foodCount'] = foodCount
    
    # Feature 2: The distance to the closest food pellet
    pacmanPosition = successor.getPacmanPosition()
    foodPositions = successor.getFood().asList()
    if foodPositions:
        closestFoodDistance = min([util.manhattanDistance(pacmanPosition, food) for food in foodPositions])
    else:
        closestFoodDistance = 0
    features['closestFoodDistance'] = closestFoodDistance
    
    # Feature 3: The number of ghosts one step away
    ...
    features['numGhostsOneStepAway'] = sum([1 for distance in ghostDistances if distance <= 1])
    
    # Feature 4: The distance to the closest ghost
    if ghostDistances:
        closestGhostDistance = min(ghostDistances)
    else:
        closestGhostDistance = 0
    features['closestGhostDistance'] = closestGhostDistance
    
    # Feature 5: Check if a Pacman is eating a capsule
    capsules = state.getCapsules()
    features['eatingCapsule'] = 1 if pacmanPosition in capsules else 0

    # Feature 6: The distance to the closest capsule
    if capsules:
        closestCapsuleDistance = min([util.manhattanDistance(pacmanPosition, capsule) for capsule in capsules])
    else:
        closestCapsuleDistance = 0
    features['closestCapsuleDistance'] = closestCapsuleDistance
    
    # Feature 7: The number of capsules left
    features['numCapsulesLeft'] = len(capsules)
    
    # Feature 8: Scared time of ghosts
    scaredTimes = [ghostState.scaredTimer for ghostState in successor.getGhostStates()]
    features['scaredTimes'] = sum(scaredTimes)
